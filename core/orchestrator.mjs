...
import { config, modelFor } from "./config.mjs";
...
export function plan(action, data = {}) {
  ...
  const variables = { "$payload": safe(data.payload, "payload"), "$optionalPayload": safe(data.payload, "payload"), "$pick": safe(data.pick, "pick", 20), "$tomorrow": berlinTomorrow() };
...
export function execute(action, data = {}) {
  const job = plan(action, data); const jobId = safe(data.jobId || `${action}-${Date.now()}`, "job id", 100).replace(/[^a-zA-Z0-9_.-]/g, "-");
  ...
  try {
    ...
    state.jobs[jobId] = { action, status: "running", startedAt: new Date().toISOString(), plan: job }; writeState(state);
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  const args = process.argv.slice(2); const action = option(args, "--action"); const data = { payload: option(args, "--payload"), pick: option(args, "--pick"), jobId: option(args, "--job-id") };
