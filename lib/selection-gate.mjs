...
const VALUE_MARKERS = [
  { re: /درآمد|پول|پرداخت|مونیتایز|monet|earn|payout|pay(?:s|out)?\b|bonus(?:es)?|income|فروش|اسپانسر|brand deal/i, label: "درآمد", weight: 10 },
  ...
  // A practical app discovery is a valid audience benefit on its own.  It is
  // intentionally below income/reach: a pretty theme with no useful outcome
  // must never outrank a verified creator-income workflow.
  { re: /اندروید|android|آیفون|iphone|آی.?اواس|iOS|مک|macOS|ویندوز|windows|لانچر|launcher|ویجت|widget|والپیپر|wallpaper|شخصی.?سازی|personaliz|کنترل با یک دست|one.?hand|یادآور|reminder|گوشی|موبایل|mobile|اپلیکیشن|app|وب.?اپ|web app/i, label: "اپ کاربردی روزمره", weight: 7 },
];
...
// Shared with content-search.mjs, which needs the same judgement before a
// candidate is even scored: a limited-access item is routed to a "محدود" news
// note instead of the selectable list (rule 3), rather than being silently
// dropped the way a hard gate rejection would. One definition, two consumers.
const LIMITED_ACCESS_RE = /(invite[\s-]?only|by invitation|select(?:ed)?\s+creators?|waitlist|pilot\s+program|early\s+access|some\s+(?:users|creators|accounts)|a\s+(?:small\s+)?(?:subset|group)\s+of|دعوتی|فقط\s*با\s*دعوت|فقط\s*برای\s*دعوت|آزمایشی|گروه\s*محدود|گروهی\s*خاص|بعضی\s*(?:کاربران|کشورها)|test\s+group|limited\s+rollout|محدود\s*به\s*منطقه|region.?limited|محدود\s*به\s*(?:کشور|منطقه)|only\s+(?:in|for)\s+[A-Z][a-z]+(?:\s+and\s+[A-Z][a-z]+)?)/i;

// Returns "limited" | "public" | "unknown" — "unknown" only when there is
// nothing to read at all, which a real search result never is, but a hand-typed
// candidate might be. Never claim "public" from silence.
export function detectAccessStatus(text) {
  const hay = String(text || "").trim();
  if (!hay) return "unknown";
  return LIMITED_ACCESS_RE.test(hay) ? "limited" : "public";
}

function audienceAvailability(c) {
  // A capability that is invite-only, region-limited, or still a small test
  // cannot be sold as a practical income/reach workflow to the general
  // audience. It may become a news item, but it is not a tutorial candidate
  // unless eligibility itself is the useful, verified subject.
  const hay = `${c.topic || ""} ${c.question || ""} ${(c.keyPoints || []).join(" ")} ${(c.notes || []).join(" ")}`;
  if (detectAccessStatus(hay) === "limited") {
    return { score: 0, why: "قابلیت برای عموم قابل‌دسترسی نیست" };
  }
  return { score: 10, why: "محدودیت عمومیِ گزارش‌شده ندارد" };
}

// Which of the three buckets content-search.mjs reports in (rule 2) a topic
// belongs to. Reuses audienceValue's own weighted labels rather than a second,
// separately-maintained keyword list that could silently drift from it —
// "دیده‌شدن"/"رشد" collapse into reach (both are the audience-size axis),
// everything else practical-and-fresh collapses into trend.
export function categorizeLane(text) {
  const hay = String(text || "");
  // Keep this ahead of the generic "trend" fallback.  The audience's recent
  // strongest response was to useful, recognisable app discoveries; they
  // deserve their own balanced slot, not an accidental leftover.
  if (/اندروید|android|آیفون|iphone|آی.?اواس|iOS|مک|macOS|ویندوز|windows|لانچر|launcher|ویجت|widget|والپیپر|wallpaper|شخصی.?سازی|personaliz|کنترل با یک دست|one.?hand|یادآور|reminder|گوشی|موبایل|mobile|اپلیکیشن|app\b|وب.?اپ|web app/i.test(hay)) return "utility";
  const hits = VALUE_MARKERS.filter((m) => m.re.test(hay));
  if (!hits.length) return "trend";
  const best = hits.reduce((a, b) => (a.weight >= b.weight ? a : b));
  if (best.label === "درآمد") return "income";
  if (best.label === "دیده‌شدن" || best.label === "رشد") return "reach";
  return "trend";
}
...
export function evaluate(candidate) {
  const axes = {
    ...
    availability: audienceAvailability(candidate),
  };
  ...
  if (axes.availability.score === 0) failures.push("قابلیت برای بیشتر مخاطبان قابل‌استفاده نیست");
