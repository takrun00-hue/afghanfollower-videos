...
const VALUE_MARKERS = [
  ...
  // A useful-phone discovery is a valid audience benefit on its own.  It is
  // intentionally below income/reach: a pretty launcher with no practical
  // outcome must never outrank a verified creator-income workflow.
  { re: /اندروید|android|لانچر|launcher|ویجت|widget|والپیپر|wallpaper|شخصی.?سازی|personaliz|کنترل با یک دست|one.?hand|یادآور|reminder|گوشی|موبایل|mobile/i, label: "کاربرد روزمرهٔ موبایل", weight: 7 },
];
...
export function categorizeLane(text) {
  ...
  // strongest response was to useful, recognisable Android discoveries; they
  // deserve their own balanced slot, not an accidental leftover.
  if (/اندروید|android|لانچر|launcher|ویجت|widget|والپیپر|wallpaper|شخصی.?سازی|personaliz|کنترل با یک دست|one.?hand|یادآور|reminder|گوشی|موبایل|mobile/i.test(hay)) return "utility";
  const hits = VALUE_MARKERS.filter((m) => m.re.test(hay));
