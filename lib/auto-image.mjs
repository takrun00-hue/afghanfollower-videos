...
export async function findRealImage(topic, points = [], usedImageUrls = new Set()) {
  ...
  for (const [strategyIndex, buildQuery] of REAL_IMAGE_SEARCH_STRATEGIES.entries()) {
    ...
    try {
      ...
      continue; // a failed request is not proof this angle has no photo — try the next distinct angle
    }
  ...
  if (!anyHits) console.error(`   ⚠ findRealImage(${topic}): Exa returned no image results across ${REAL_IMAGE_SEARCH_STRATEGIES.length} distinct search strategies`);
  return null;
}
