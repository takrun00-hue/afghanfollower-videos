...
export async function findRealImage(topic, points = [], usedImageUrls = new Set()) {
  if (!EXA_KEY || (!GEMINI_KEY && !GROQ_KEY)) {
    console.error(`   ⚠ findRealImage(${topic}): no ${!EXA_KEY ? "EXA_API_KEY" : "GEMINI_API_KEY/GROQ_API_KEY"} configured — cannot search`);
    return null;
  }
  let hits = [];
  try {
    ...
    console.error(`   ⚠ findRealImage(${topic}): exaSearch failed — ${e.message}`);
    return null;
  }
  if (!hits.length) console.error(`   ⚠ findRealImage(${topic}): Exa returned no image results`);
  ...
  if (hits.length) console.error(`   ⚠ findRealImage(${topic}): ${hits.length} candidates, none passed — used:${rejected.used} download:${rejected.download} badType:${rejected.type} tooSmall:${rejected.size} notRelevant:${rejected.irrelevant}`);
  return null;
}
...
export async function generateAIImage(topic, slideText) {
  ...
  try {
    ...
    console.error(`   ⚠ generateAIImage(${topic}): request failed — ${e.message}`);
    return null;
  }
  if (!response.ok) {
    const errBody = await response.text().catch(() => "");
    console.error(`   ⚠ generateAIImage(${topic}): Gemini ${response.status} — ${errBody.slice(0, 300)}`);
    return null;
  }
  ...
  if (!b64) {
    ...
    console.error(`   ⚠ generateAIImage(${topic}): no image data in response${blockReason ? ` (${blockReason})` : ""}`);
    return null;
  }
  ...
  const type = imageType(rawFile);
  if (!type) {
    console.error(`   ⚠ generateAIImage(${topic}): downloaded data is not a recognized image type`);
    return null;
  }
  const named = rawFile.replace(/\.img$/, `.${type === "jpeg" ? "jpg" : type}`);
  writeFileSync(named, bytes);
  ...
  if (!size || Math.max(size.width, size.height) < 1080 || size.width * size.height < 700000) {
    console.error(`   ⚠ generateAIImage(${topic}): generated image too small (${size ? `${size.width}x${size.height}` : "unreadable"})`);
    return null;
  }
