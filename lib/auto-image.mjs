...
//
// The owner also asked, in the same report, for a "guessed" image to stand
// in when a real per-slide photo can't be found. Not done: PROJECT_RULES.md
// rule 39 requires a REAL photo/video of the same topic per slide and rule
// 41 bans exactly this ("رابط حدسی ... تصویر نامرتبط" / invented UI, an
// unrelated stand-in image) — and CLAUDE.md's own priority order for this
// project ranks "واقعیت و ایمنی اطلاعات" (reality/factual safety) ABOVE
// "تأیید صاحب کانال" (channel-owner approval), so this is not a call an
// agent gets to make differently just because the owner asked in chat. A
// slide with no real photo found stays without one; the existing Visual
// Truth Gate (assertVisualProof, called by every caller of this function)
// still rejects it and daily-render.mjs's own retry loop swaps to a fresh
// topic — same as before, never a fabricated stand-in.
export async function rescuePackPhotos(pack) {
  ...
  for (let i = 0; i < missing.length; i++) {
    const tip = missing[i];
    const found = await findRealImage(topic, [textOf(tip)].filter(Boolean), usedImageUrls);
    if (!found) continue; // no real photo for this slide — left empty, never a guessed stand-in
    if (found.imageUrl) usedImageUrls.add(found.imageUrl);
    tip.photo = found.photo;
    tip.photoFocus = focuses[i % focuses.length];
    tip.visualEvidence = {
      sourceUrl: found.sourceUrl,
      sourceType: found.sourceType,
      claim: textOf(tip).slice(0, 140),
    ...
    filled++;
    if (!pack.source && !pack.sources?.length && found.sourceUrl) pack.source = found.sourceUrl;
  }
  return filled > 0;
}
