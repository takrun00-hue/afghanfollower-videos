...
// These two public catalogues require neither an API key nor a paid plan.
// Their results are intentionally limited to Public Domain / CC0: the system
// must never silently reuse a CC-BY image when no attribution card exists.
export const FREE_IMAGE_SEARCH_PROVIDERS = ["wikimedia-commons", "openverse"];
const FREE_LICENSES = new Set(["cc0", "pdm", "public domain", "publicdomain", "cc public domain"]);
function isFreeReusableLicense(value) {
  const normal = String(value || "").toLowerCase().replace(/[\s_-]+/g, " ").trim();
  return FREE_LICENSES.has(normal);
}

async function wikimediaCommonsSearch(query) {
  if (providerUnavailable("wikimedia-commons")) return [];
  const params = new URLSearchParams({
    action: "query", generator: "search", gsrsearch: query, gsrnamespace: "6", gsrlimit: "8",
    prop: "imageinfo", iiprop: "url|size|extmetadata", iiurlwidth: "1440", format: "json", origin: "*",
  });
  let response;
  try { response = await fetch(`https://commons.wikimedia.org/w/api.php?${params}`); } catch { return []; }
  if (!response.ok) {
    rememberProviderFailure("wikimedia-commons", response.status);
    return [];
  }
  ...
  return pages.map((page) => {
    ...
    if (!info.thumburl || !isFreeReusableLicense(license)) return null;
    const title = String(page.title || "");
    return {
      image: info.thumburl, title: title.replace(/^File:/, ""),
      url: `https://commons.wikimedia.org/wiki/${encodeURIComponent(title.replace(/ /g, "_"))}`,
      snippet: `Wikimedia Commons — ${license}`, provider: "wikimedia-commons", license,
    };
  }).filter(Boolean);
}

async function openverseSearch(query) {
  if (providerUnavailable("openverse")) return [];
  const params = new URLSearchParams({ q: query, page_size: "8", license: "cc0,pdm" });
  let response;
  try { response = await fetch(`https://api.openverse.org/v1/images/?${params}`); } catch { return []; }
  if (!response.ok) {
    rememberProviderFailure("openverse", response.status);
    return [];
  }
  return ((await response.json())?.results || []).map((item) => {
    if (!item?.url || !isFreeReusableLicense(item.license)) return null;
    return {
      image: item.url, title: item.title || "", url: item.foreign_landing_url || item.detail_url || item.url,
      snippet: `Openverse — ${item.license}`, provider: "openverse", license: item.license,
    };
  }).filter(Boolean);
}

function lexicalRelevance(topic, points, candidate) {
  const words = `${topic} ${points.join(" ")}`.toLowerCase().match(/[\p{L}\p{N}]{4,}/gu) || [];
  const haystack = `${candidate.title} ${candidate.url} ${candidate.snippet}`.toLowerCase();
  // Used only for metadata of a Public Domain / CC0 candidate. The stronger
  // Gemini/Groq check remains the preferred route for ordinary web results.
  return [...new Set(words)].some((word) => haystack.includes(word));
}
...
export async function findRealImage(topic, points = [], usedImageUrls = new Set()) {
  let hits = [];
  try {
    // Keep room for the free providers even when an Exa page returns many
    // unsuitable thumbnails; otherwise they could never be examined.
    hits.push(...(await exaSearch(`${topic} official screenshot OR interface OR product page OR press photo`)).slice(0, 3));
  } catch { /* Continue with the free no-key sources below. */ }
  const publicQuery = `${topic} ${points.join(" ")}`.trim();
  const [commons, openverse] = await Promise.all([wikimediaCommonsSearch(publicQuery), openverseSearch(publicQuery)]);
  hits.push(...commons.slice(0, 3), ...openverse.slice(0, 3));
  if (!hits.length) return null;
  mkdirSync("public/user-media", { recursive: true });
  for (const hit of hits.slice(0, 9)) {
    ...
    const candidate = { title: String(hit.title || "").slice(0, 160), url: hit.url, snippet: String(hit.text || hit.snippet || "").slice(0, 260) };
    let relevant = false;
    try { relevant = await isRelevant(topic, points, candidate); } catch { relevant = false; }
    if (!relevant && hit.provider && lexicalRelevance(topic, points, candidate)) relevant = true;
    if (!relevant) continue;

    const sourceType = hit.provider === "wikimedia-commons" ? "commons-licensed"
      : hit.provider === "openverse" ? "openverse-licensed" : "labelled-explainer";
    return { photo: named, sourceUrl: hit.url, sourceType, alt: candidate.title || topic, imageUrl: hit.image };
  }
  return null;
}
