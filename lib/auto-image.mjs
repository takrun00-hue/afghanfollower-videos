...
// guessed `tip.ui` mockup, never a real screenshot. One shared search
// covers the whole pack (all missing slides get the same found photo, with
// a different photoFocus each — the same reuse pattern custom-content.mjs
// already uses for its own auto-found image) rather than one search per
// slide, so this stays fast enough to run inside daily-render.mjs's
// per-candidate retry loop. Returns true if anything was actually filled in.
export async function rescuePackPhotos(pack) {
  ...
  const points = missing.map(textOf).filter(Boolean);
  ...
  const found = userPhoto
    ? { photo: userPhoto, sourceUrl: "ارسال مستقیم صاحب کانال از طریق تلگرام", sourceType: "owner-supplied", alt: `اسکرین‌شات واقعی ${topic} — ارسال‌شده توسط صاحب کانال` }
    : await findRealImage(topic, points);
