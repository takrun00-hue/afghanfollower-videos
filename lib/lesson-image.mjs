...
import { findPixabayImage } from "./pixabay-image.mjs";
...
export async function findLessonImage(query, label, ownAssetKey = "") {
  ...
  // LAYER 2: Pixabay — the other free, no-card stock library. A genuinely
  // different catalogue, so it is worth asking when Pexels' own index had
  // nothing for this concept. Same verification, same floor, same relevance
  // gate; a second free provider never means a second, weaker bar.
  const fromPixabay = await findPixabayImage(words.join(" "), label);
  if (fromPixabay) return fromPixabay;

  // LAYER 3: Wikipedia/Wikimedia — the only source here that needs no key
  ...
  // LAYER 4: Exa. Kept, not deleted — it is a genuinely different index,
