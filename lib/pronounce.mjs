...
const LIGHT_VERB = new Set(["کن", "بده", "بزن", "ببین", "بخوان", "بساز", "کنی", "دهد",
...
function breathe(text) {
  return String(text).split(/([؟!.])/).map((part) => {
    ...
    for (let i = 0; i < w.length; i++) {
      out.push(w[i]);
      since++;
      const last = w[i].replace(/،/g, "");
      const next = (w[i + 1] || "").replace(/،/g, "");
...
export function minimaxSpeakable(text) {
  ...
  const PERSIAN_TTS_FIXES = [
    ...
    // A marked ending after a space is heard as its own syllable: «تیک تاکَت»
    // came out as «تیک تاک کَت». Persian says this as two words, so the spoken
    ["تیک تاکت", "تیک تاکِ تو"],
    ["تیک‌تاکت", "تیک تاکِ تو"],
    ["اینستاگرامت", "اینستاگرامِ تو"],
