...
export function minimaxSpeakable(text) {
  ...
  const PERSIAN_TTS_FIXES = [
    ...
    // «تیک‌تاکت» is TikTok with the possessive attached, and without the vowel
    // mark the engine runs the two together into something that is not a word.
    ["تیک تاکت", "تیک تاکَت"],
    ["تیک‌تاکت", "تیک تاکَت"],
    ["اینستاگرامت", "اینستاگرامَت"],
