...
export function minimaxSpeakable(text) {
  ...
  const PERSIAN_TTS_FIXES = [
    ...
    ["صفحه‌ات", "صَفحه اَت"],
    ["ویوهایت", "ویوهایَت"],
    ["پیامت", "پیامَت"],  // «نه لای الگوریتم» proved «نه لای» itself breaks the engine across every take tested; this mark is a smaller, separate fix — the possessive on «پیام» — kept regardless of how the sentence around it is worded.
    ["ویوهای", "ویوهایِ"],
