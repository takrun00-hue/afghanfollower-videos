...
const VO = {
  ...
  "a1-17-adjectives": {
    hook: "بزرگ یا کوچک، خوب یا بد؛ این صفت‌های پرکاربرد را همین امروز یاد بگیر.",
    steps: [
      "این دو کلمه یعنی بزرگ و کوچک؛ همیشه در تضاد با هم یاد بگیرشان.",
      // Was "این دو کلمه یعنی خوب و بد." — production incident 2026-09-13:
      // a bare, sentence-final «بد» (one syllable, no trailing context) was
      // heard as «بعد» or «برد» on 3 of 4 independent synthesis takes across
      // two builds, repeatedly blocking Narration QC. Not a same-sound case
      // (بد/بعد/برد are distinct words with different meanings — folding
      // them in lib/hear.mjs would risk hiding a real word-substitution
      // defect on the actual vocabulary this unit teaches), so the fix is
      // giving the TTS/ASR round-trip more phonetic context instead, the
      // same way every other step in this unit already does.
      "این دو کلمه یعنی خوب و بد؛ همیشه در مقابل هم به کار می‌روند.",
