...
const defaultGrokKey = process.env.GROK_API_KEY || process.env.XAI_API_KEY || "";
...
export async function translateToPersian(items, {
  ...
  if (!openRouterKey && !grokKey) throw new Error("کلید OPENROUTER_API_KEY یا GROK_API_KEY برای برگردان فارسی تنظیم نشده است.");
  ...
  async function request(url, key, model, provider) {
    const response = await fetch(url, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        authorization: `Bearer ${key}`,
        ...(provider === "openrouter" ? { "x-title": "GapMedia Content Research" } : {}),
      },
      body: JSON.stringify({
        model,
        temperature: 0.1,
  ...
  try {
    // OpenRouter is the primary Persian translation provider. Grok remains a
    // compatibility fallback, so a temporary OpenRouter quota/model problem
    // never sends untranslated search cards to Telegram.
    let translated;
    try {
      if (!openRouterKey) throw new Error("OpenRouter key unavailable");
      translated = await request("https://openrouter.ai/api/v1/chat/completions", openRouterKey, openRouterModel, "openrouter");
    } catch (openRouterError) {
      if (!grokKey) throw openRouterError;
      translated = await request("https://api.x.ai/v1/chat/completions", grokKey, "grok-4-1-fast-reasoning", "grok");
    }
