...
export async function translateToPersian(items, {
  ...
  // the order.
  const providers = [];
  if (geminiKey) providers.push({ name: "Gemini", run: (strict) => callGemini(buildPrompt(targets, { strict }), geminiKey, geminiModel) });
  if (groqKey) providers.push({ name: "Groq", run: (strict) => callGroq(buildPrompt(targets, { strict }), groqKey, groqModel) });

  let translated = null, runProvider = null, lastErr = null;
  for (const provider of providers) {
    try {
      translated = parseArray(await provider.run(false));
      runProvider = provider.run;
      break;
    } catch (e) {
      lastErr = e;
      console.error(`${provider.name} translation failed (${e.message}); ${provider === providers[providers.length - 1] ? "no fallback left" : "trying the next provider"}`);
    }
  }
  if (!translated) throw lastErr || new Error("no translation provider configured");
