...
function leastRecentlyUsed(arr, staleness) {
  let best = arr[0], bestT = Infinity;
...
export function featureFor(cat, dayIndex, exclude = new Set(), staleness = null) {
  ...
  // raw pick (see leastRecentlyUsed() above).
  return pickAvoiding(list, i, exclude) || (staleness ? leastRecentlyUsed(list, staleness) : list[i]);
}
...
export function aiFeatureFor(dayIndex, exclude = new Set(), staleness = null) {
  ...
  return pickAvoiding(candidates, i, exclude) || (staleness ? leastRecentlyUsed(candidates, staleness) : candidates[i]);
}
