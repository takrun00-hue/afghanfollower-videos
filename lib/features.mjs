...
export function featureFor(cat, dayIndex, exclude = new Set()) {
  const list = [
    ...((DEMAND && DEMAND[cat]) || []),
  ...
  // Every id in the category is excluded — nothing left to swap to, so repeat
  // the day's own pick rather than return nothing.
  return pickAvoiding(list, i, exclude) || list[i];
}
...
export const AI_DAILY_IDS = Object.freeze([
  "kling-ai", "adobe-podcast", "suno", "elevenlabs", "descript", "ai-hook-writer",
]);

export function aiFeatureFor(dayIndex, exclude = new Set()) {
  ...
  // every one of them is excluded, repeat the day's own pick.
  return pickAvoiding(candidates, i, exclude) || candidates[i];
}
