...
const SCENES = {
  ...
  // a toolbox / creator-tools section. The hammer that used to sit on the lid
  // collided with the handle; the tools now peek out of the box instead.
  toolbox: () => `${ground(120, 86)}
    <path class="mfx" d="M88 106 v-16 a32 32 0 0 1 64 0 v16" fill="none" stroke="var(--d)" stroke-width="12"/>
    <rect class="mfx" x="112" y="52" width="16" height="34" rx="8" fill="var(--d)"/>
    <rect class="mfx" x="154" y="60" width="18" height="46" rx="9" fill="var(--d)" opacity=".55"/>
    <rect class="mfx" x="30" y="106" width="180" height="96" rx="16" fill="var(--a)"/>
    <rect class="mfx" x="30" y="106" width="180" height="22" rx="10" fill="#fff" opacity=".28"/>
    <rect class="mfx" x="104" y="132" width="32" height="38" rx="8" fill="#fff"/>
    <circle class="mpulse" cx="120" cy="151" r="7" fill="var(--d)"/>`,
  ...
  star: () => `${ground(120, 78)}
    <circle class="mfx" cx="120" cy="118" r="82" fill="var(--a)" opacity=".14"/>
    <path class="mfx" d="M120 46 l24 50 55 8 -40 39 10 55 -49 -27 -49 27 10 -55 -40 -39 55 -8z" fill="var(--a)"/>
    <path class="mpulse" d="M198 56 l5 11 11 5 -11 5 -5 11 -5 -11 -11 -5 11 -5z" fill="var(--d)"/>`,

  // a heart / follow / like
  heart: () => `${ground(120, 76)}
    <circle class="mfx" cx="120" cy="120" r="82" fill="var(--a)" opacity=".13"/>
    <path class="mpulse" d="M120 190 c-58 -38 -74 -66 -74 -92 a38 38 0 0 1 74 -14 a38 38 0 0 1 74 14 c0 26 -16 54 -74 92z" fill="var(--a)"/>`,
...
export function sceneArtFor(tip) {
  ...
  if (has("بنویس", "کپشن", "اطلاعات تماس", "توضیح")) return "write";
...
export function stepStage(PAIR, { tip, art, markSVG = "", iconSVG = "" }) {
