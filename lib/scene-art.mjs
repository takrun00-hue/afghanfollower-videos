...
export function sceneArtFor(tip) {
  ...
  if (has("پیدا می‌شوی", "پیدا شوی", "بالا می‌آید", "بالا بیاید", "در جستجو")) return "found";
...
export function hookArtFor(pack) {
  // A news hook has no l1/l2 to read — the headline IS the hook, so the art has
  // to come from it or every story gets the same meaningless circle.
  const text = pack.platform === "news"
    ? (pack.hook?.ask || "")
