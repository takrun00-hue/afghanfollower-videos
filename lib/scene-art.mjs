...
export function hookArtFor(pack) {
  // A German-lesson hook has no l1/l2 to read — hook.ask IS the hook text, so
  // the art has to come from it or every episode gets the same meaningless
  // circle. (Formerly gated on the now-retired "news" platform, which this
  // pack used to borrow for its unbranded layout.)
  const text = pack.kind === "german-lesson"
    ? (pack.hook?.ask || "")
    : `${pack.hook?.l1 || ""} ${pack.hook?.l2 || ""}`;
