...
// `body` is trusted, already-escaped SSML content (may contain real markup,
// e.g. <lang> for a mid-sentence language switch); `plainText` is escaped
// here. Exactly one of the two is used.
function mkssml({ text, body, voice, rate, pitch, volume }) {
  return (
    `<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xml:lang='fa-IR'>` +
    `<voice name='${voice}'>` +
    `<prosody pitch='${pitch}' rate='${rate}' volume='${volume}'>` +
    `${body != null ? body : escapeXml(text)}` +
    `</prosody></voice></speak>`
  );
}

// One voice request. Resolves with the raw MP3 bytes.
export function synthesize({ text, body, voice = "fa-IR-FaridNeural", rate = "+0%", pitch = "+0Hz", volume = "+0%", timeoutMs = 20000 }) {
      ...
      socket.write(
        encodeFrame(
          ...
          `X-RequestId:${connectId}\r\n` +
            ...
            `X-Timestamp:${ts}Z\r\n` +
            `Path:ssml\r\n\r\n` +
            mkssml({ text, body, voice, rate, pitch, volume })
        )
      );
    });
    req.end();
