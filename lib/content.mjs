...
function buildFeaturePack(cat, dayIndex, variant, bpm, beat, forced) {
  ...
  return {
    ...
    tips: (() => {
      // Two phone screens in a row read as the same picture twice — same chrome,
      // same list, only the words differ. Keep the first one and let the rest use
      // their own concept art, so four slides stay four distinct images.
      let usedScreen = false;
      return steps.map((s, i) => {
        const screen = s.screen && !usedScreen ? ((usedScreen = true), s.screen) : undefined;
        return { icon: s.icon, step: i + 1, head: s.text, ui: s.ui, path: s.path, screen, brand: s.brand };
      });
    })(),
    payoff: f.payoff,
...
export function packForFeature(id, date = new Date(), generated = null) {
  // a one-off news item written by news-build.mjs
  if (generated) {
    const di = Math.floor(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()) / 86400000);
    return buildFeaturePack("news", di, 1, 120, 0.5, generated);
  }
