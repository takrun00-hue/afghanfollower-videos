...
const THEMES = {
  ...
  instagram: {
    ...
    emc: "#7C3AED", accents: ["#D946EF", "#8B5CF6", "#A78BFA"],
  },
  general: {
...
const INK = {
  instagram: { pair: ["#5B21B6", "#D946EF"], paper: "#F7F7FC", tint: "rgba(91,33,182,.10)" },
...
function buildFeaturePack(cat, dayIndex, variant, bpm, beat, forced) {
  ...
  const plan = beatPlan(seed, isNews ? 27 : SHORT ? 16 : 56, steps.length, mood);
  ...
  return {
    ...
    tips: steps.map((s, i) => ({
      ...
      screen: s.screen, brand: s.brand, photo: s.photo, photoFocus: s.photoFocus,
    })),
    payoff: f.payoff,
