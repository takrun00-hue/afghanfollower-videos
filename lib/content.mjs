...
function buildFeaturePack(cat, dayIndex, variant, bpm, beat, forced) {
  const f = forced || (cat === "tools" ? aiFeatureFor(dayIndex) : featureFor(cat, dayIndex));
  if (!f) return null;
...
export function dailyDeliveriesForDate(date = new Date()) {
  const selected = packsForDate(date);
