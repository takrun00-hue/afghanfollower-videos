...
const INK = {
  instagram: { pair: ["#0B4F63", "#0E7C9B"], paper: "#FBF6EF", tint: "rgba(11,79,99,.10)" },
  ...
  news:      { pair: ["#12171F", "#E01B26"], paper: "#F2F1EF", tint: "rgba(224,27,38,.07)" },
};
...
// mutating research/content records. The German Insider news channel bypasses
// this function and keeps its separate identity.
function currentBrandText(value, category) {
  if (category === "news") return value;
  return String(value || "")
...
function currentHook(feature) {
  ...
  if (feature.id === "saved-replies") {
    hook.ask = "دایرکت فروش می ماند، مشتری می رود؟ تا آخر ببین؛ چهار مرحله را می گویم.";
    hook.l1 = "دایرکت فروش می ماند،";
    hook.l2 = "مشتری می رود؟";
  }
  if (feature.id === "ig-insights-retention") {
    hook.ask = "ویوت یک دفعه می ریزد؟ تا آخر ببین؛ علتش را پیدا می کنی.";
...
function buildFeaturePack(cat, dayIndex, variant, bpm, beat, forced) {
  ...
  if (!f) return null;
  // A tip is one action and reads in fifteen seconds. A news story is a short
  // bulletin: the headline, then the two or three facts that actually matter.
  // Four cards land it inside half a minute — long enough to be a story, short
  // enough that the viewer finishes it.
  const isNews = cat === "news";
  const NEWS_STEPS = 4;
  const steps = f.steps.slice(0, isNews ? NEWS_STEPS : TIPS_PER_VIDEO);
  ...
  const plan = beatPlan(seed, Math.round((isNews ? 27 : SHORT ? 16 : 56) * pace), steps.length, mood);
  ...
  return {
    ...
    platform: cat,
    theme: THEMES[cat],
    title: currentBrandText(f.title, cat),
    feature: f.name,
    hookPhoto: f.hookPhoto || null,
    ...
    tgTitle: currentBrandText(f.tgTitle, cat),
    kicker: f.kicker || KICKERS[cat] || "قابلیت",
    ink: design.ink || INK[cat] || INK.tools,
    design,
    layoutSeed: 0,
    ...
    tips: steps.map((s, i) => ({
      ...
      screen: s.screen, brand: s.brand, photo: s.photo, photoFocus: s.photoFocus, photoAlt: s.photoAlt, video: s.video, videoStart: s.videoStart,
    })),
    payoff: f.payoff,
...
export function packForFeature(id, date = new Date(), generated = null) {
  // One-off items written by news-build.mjs or custom-content.mjs. The
  // generator declares its category, so a user-supplied tutorial remains a
  // tutorial and never inherits the German Insider news treatment.
  if (generated) {
    const di = Math.floor(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()) / 86400000);
    return buildFeaturePack(generated.category || "news", di, 1, 120, 0.5, generated);
  }
  // news lives in its own pack, outside the daily rotation
  for (const f of NEWS.news || []) {
    if (String(f.id).toLowerCase() === String(id).toLowerCase()) {
      const di = Math.floor(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()) / 86400000);
      return buildFeaturePack("news", di, 1, 120, 0.5, f);
    }
  }
...
OUTRO.news = { tag: 'خبرهای آلمان و اروپا،<br/>دقیق و سریع.', follow: "دنبال کنید +" };
KICKERS.news = "خبر";
// German Insider: red on near-black, the logo's own palette.
THEMES.news = {
  grad: "linear-gradient(160deg,#2A0A0E 0%,#12171F 46%,#05070A 100%)",
  gradOutro: "linear-gradient(160deg,#E01B26 0%,#8E1119 42%,#12171F 100%)",
  glow1: "rgba(224,27,38,.38)", glow2: "rgba(18,23,31,.34)",
  emc: "#C81420", accents: ["#E01B26", "#F5F5F5", "#C81420"],
};
...
const TOOL_CATEGORIES = [
  { match: /چت|گفتگو|مکالمه/, nameFa: "برای چت معمولی", tools: ["ChatGPT", "Claude", "Gemini"] },
  { match: /تحقیق|پژوهش|جست ?و ?جو/, nameFa: "برای تحقیق", tools: ["Perplexity", "Claude", "ChatGPT"] },
  { match: /تصویر|عکس/, nameFa: "برای ساخت تصویر", tools: ["Midjourney", "Ideogram", "DALL·E"] },
  { match: /ویدیو|فیلم/, nameFa: "برای ساخت ویدیو", tools: ["Sora", "Kling AI", "Higgsfield"] },
  { match: /صدا|موسیقی|آهنگ/, nameFa: "برای صدا و موسیقی", tools: ["Suno", "ElevenLabs"] },
  { match: /نوشتن|متن|کپشن/, nameFa: "برای نوشتن متن", tools: ["Claude", "ChatGPT", "Gemini"] },
  { match: /کد|برنامه ?نویسی/, nameFa: "برای برنامه‌نویسی", tools: ["Claude", "ChatGPT"] },
];

// The spoken copy is not the written copy (persian-tts-reading skill,
// foreign-words.md): the pill still shows "ChatGPT" in Latin, but a listener
// reported fa-IR-DilaraNeural reading English names in no recognisable
// language. Every tool this pack can name is respelled here now, not just
// the one first flagged — Whisper-based verification is blocked on this
// machine (an Application Control policy blocks a numba DLL check-voice.mjs
// depends on), so none of this is confirmed by listening the way «ریلزِت»
// was in VOICE-LOG.md. Send fresh audio and listen before trusting these.
//
// never-split.md: a respelling that becomes multiple Persian words on the
// page must join with a NON-BREAKING space, or a pause can land inside the
// name — the exact fault this whole map exists to prevent, just moved one
// level in. Only ChatGPT's respelling has more than one word.
const NBSP = " ";
export const TOOL_PRONOUNCE = {
  ChatGPT: `چت${NBSP}جی${NBSP}پی${NBSP}تی`,
  Claude: "کلاود",
  Gemini: "جمینای",
...
/** Swap known brand names into their verified spoken form before TTS. Longest
 * name first so "Kling AI" isn't partly eaten by a shorter "Kling" match. */
export function spokenForm(text) {
  let out = String(text || "");
  for (const name of Object.keys(TOOL_PRONOUNCE).sort((a, b) => b.length - a.length)) {
    out = out.replace(new RegExp(`\\b${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "g"), TOOL_PRONOUNCE[name]);
  }
  return out;
}
...
export function tierListPack(inputText) {
  ...
  for (const clause of clauses) {
    ...
    seen.add(cat.nameFa);
    categories.push({ nameFa: cat.nameFa, tools: cat.tools });
  }
  ...
  const HOOK_DUR = 4, OUTRO_DUR = 4, PER_SLIDE = 5;
  return {
    id: "tier-list",
    platform: "tools",
    tierList: true,
    theme: THEMES.ai,
    title: "بهترین ابزار هوش مصنوعی برای هر کار",
    kicker: "رتبه‌بندی ابزارها",
    hook: {
      badge: "🔥 رازهای AI",
      ...
      // a 3-category input must not claim "۹ ابزار" when it only has 6.
      parts: [
        { text: "چت جی پی تی تموم شد؟ ", cls: "tier-part-plain" },
        { text: `${FA(categories.reduce((n, c) => n + c.tools.length, 0))} ابزار`, cls: "tier-part-purple" },
        { text: " بهتر ", cls: "tier-part-plain" },
        { text: "رایگان", cls: "tier-part-green" },
      ],
      tail: "",
    },
    tgTitle: "🏆 بهترین ابزار هوش مصنوعی برای هر کار\n\n#هوش_مصنوعی #ابزار #GapMedia #viral",
    ...
    narration: [
      ...
      `ChatGPT کافی نیست، این ${persianNumberWords(categories.reduce((n, c) => n + c.tools.length, 0))} تا رایگان و قوی‌ترن.`,
      // breath-and-rhythm.md: a breath after one or two phrases, never a run
      // of three names in one push. Each tool gets its own comma — including
      // before "یا" and before the predicate, which used to follow the last
      // name with no breath at all.
      ...categories.map((c) => {
        const last = c.tools[c.tools.length - 1];
        const rest = c.tools.slice(0, -1).join("، ");
        return `${c.nameFa}، ${rest ? rest + "، یا " : ""}${last}، گزینه‌های خوبن.`;
      }),
      "برای محتوای حرفه‌ای‌تر، ما رو دنبال کن.",
    ],
  };
}
