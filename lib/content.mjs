...
function buildFeaturePack(cat, dayIndex, variant, bpm, beat, forced) {
  ...
  return {
    ...
    hook: { badge: f.name, ask: f.hook.ask || (() => { console.error(`   ⚠ ${f.id} has no benefit hook — using a weak fallback`); return askFor(cat, f.id, dayIndex); })(), l1: f.hook.l1, l2: f.hook.l2, benefit: f.benefit || null },
