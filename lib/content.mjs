...
export function tierListPack(inputText) {
  ...
  return {
    ...
    hook: {
      badge: "🔥 رازهای AI",
      // Three styled phrases typed in sequence (red/shake, green/scale,
      // gold/glow), then a plain trailing emoji. The count is computed from
      // the real pack contents — a 3-category input must not claim "۹ ابزار"
      // when it only has 6.
      parts: [
        { text: "ChatGPT تمام شد!", cls: "tier-part-bad" },
        { text: `${FA(categories.reduce((n, c) => n + c.tools.length, 0))} ابزاری که جایش را گرفت`, cls: "tier-part-good" },
        { text: "رایگان و ۱۰ برابر قوی‌تر", cls: "tier-part-gold" },
      ],
      tail: " 👇",
    },
