...
// reported fa-IR-DilaraNeural reading it in no recognisable language.
// Respelled here for the ones a listener actually flagged — a name not in
// this map is left in Latin (the previously verified default for generic
// English), not guessed at, since an invented respelling could read worse
...
export function spokenForm(text) {
  ...
  for (const name of Object.keys(TOOL_PRONOUNCE).sort((a, b) => b.length - a.length)) {
    out = out.replace(new RegExp(`\\b${name}\\b`, "g"), TOOL_PRONOUNCE[name]);
  }
  return out;
}
...
export function tierListPack(inputText) {
  ...
  return {
    ...
    // Same bed the dialogue pack used — render-tiktok-remake.mjs only mixes
    // it in with --with-music; ducked under the voice, never silent-default.
    music: "music/bed-60s-v1.m4a",
    outro: OUTRO.tools,
    ...
    narration: [
      ...
      `دیگه فقط ChatGPT کافی نیست؛ این ${FA(categories.reduce((n, c) => n + c.tools.length, 0))} ابزار رایگان و چند برابر قوی‌ترن.`,
      ...categories.map((c) => {
