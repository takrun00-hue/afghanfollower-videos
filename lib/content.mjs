...
function buildFeaturePack(cat, dayIndex, variant, bpm, beat, forced) {
  ...
  if (!f) return null;
  // A tip is one action and reads in fifteen seconds. A news story has to
  // carry what happened, who said what, and what it does not mean — that needs
  // a minute, and more scenes to put it in.
  ...
  const plan = beatPlan(seed, isNews ? 62 : SHORT ? 16 : 56, steps.length, mood);
