...
// ----------------------------------------------------------------------
// Faceless tool tier-list. No character, no voiceover — the reference format
// this copies (e.g. "The Definitive Ranking") is silent: big Persian
// captions and a tool name popping into a pill are the entire video.
//
...
export function tierListPack(inputText) {
  ...
  return {
    ...
    hook: {
      badge: "🔥 راهنمای ابزار",
      stat: "۹۰٪ آدما",
      l1: "هوش مصنوعی رو اشتباه",
      l2: "استفاده میکنن!",
      // The real count for THIS pack, not a copied-over number — a 3-category
      // input must not claim "۹ تا" when it only has 6.
      sub: `این ${FA(categories.reduce((n, c) => n + c.tools.length, 0))} تا رو سیو کن 👇`,
    },
