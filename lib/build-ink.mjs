...
export function buildInkHTML(pack) {
  ...
  const scenes = pack.tips
    ...
    <span class="charwrap">${characterSVG({ pose: poseFor(tip.head || ""), ink })}</span>
...
/* Warm off-white with an orange cast, not a blue-grey one. The page reads as
   ...
   apricot at the edges, with a faint grid for scale. */
.scene:has(.official-ui) .paper{
  background:
    radial-gradient(circle at 50% 36%, #fffdfa, transparent 56%),
    radial-gradient(120% 88% at 86% 2%, rgba(255,196,138,.34), transparent 58%),
    radial-gradient(100% 70% at 6% 100%, rgba(255,214,170,.30), transparent 60%),
    linear-gradient(176deg,#fffaf4 0%,#fdf0e2 48%,#f7e3cf 100%)}
  .scene:has(.official-ui) .paper::before{content:"";position:absolute;inset:0;opacity:.5;
  background-image:linear-gradient(rgba(150,110,70,.09) 1px,transparent 1px),
    linear-gradient(90deg,rgba(150,110,70,.09) 1px,transparent 1px);
