...
export function buildInkHTML(pack) {
  ...
  const wideShot = (photo) => {
    const p = photo?.src || photo;
    if (typeof p !== "string") return false;
    const size = imageSize(p);
    return !!size && size.width / size.height > 1.2;
  };
  ...
  const sceneStep = (i, tip, ink) => `
      <div class="stepwrap">
        ...
        ${tip.video ? null : tip.photo
          ? `<div class="official-ui ${wideShot(tip.photo) ? "wide" : ""} focus-${tip.photoFocus || "default"}"><img src="${photoSrc(tip.photo)}" alt="${tip.photoAlt || "Official app interface"}"/><span class="focusframe"></span><span class="tapdot"></span></div>`
          : pack.photos?.[i]
...
.official-ui.wide{width:820px;height:496px;border-radius:26px;background:#0c1220}
.official-ui.wide img{object-fit:contain}
.official-ui.wide .focusframe,.official-ui.wide .tapdot{display:none}
