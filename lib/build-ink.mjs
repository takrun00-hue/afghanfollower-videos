  ...
  const sceneStep = (i, tip, ink) => `
      <div class="stepwrap">
        ...
        ${tip.video ? null : tip.photo
          ? `<div class="official-ui ${shapeOf(tip.photo).cls} focus-${tip.photoFocus || "default"}" style="--ar:${shapeOf(tip.photo).ar.toFixed(3)}">${shapeOf(tip.photo).cls === "phone" ? '<span class="island"></span><span class="btn-l"></span><span class="btn-r"></span>' : ""}<span class="shot"><img src="${photoSrc(tip.photo)}" alt="${tip.photoAlt || "Official app interface"}"/></span><span class="focusframe"></span><span class="tapdot"></span></div>`
          : pack.photos?.[i]
...
.scene:has(.official-ui) .paper{
  background:
    radial-gradient(circle at 50% 34%, color-mix(in srgb,var(--ink) 42%,transparent), transparent 56%),
    radial-gradient(circle at 78% 6%, rgba(120,160,220,.16), transparent 44%),
    linear-gradient(178deg,#141a24 0%,#0d1119 46%,#07090d 100%)}
.scene:has(.official-ui) .paper::after{background:none}
.scene:has(.official-ui) .paper::before{content:"";position:absolute;inset:0;opacity:.5;
  background-image:linear-gradient(rgba(150,180,225,.10) 1px,transparent 1px),
    linear-gradient(90deg,rgba(150,180,225,.10) 1px,transparent 1px);
  background-size:68px 68px;
  mask-image:radial-gradient(circle at 50% 40%, #000 10%, transparent 74%)}
.scene:has(.official-ui) .grain{opacity:.10}
/* Type has to invert with the ground or it disappears into it. */
.scene:has(.official-ui) .kick{color:#93a2b8}
.scene:has(.official-ui) .cap{color:#e8edf5;background:rgba(255,255,255,.06);
  border-color:rgba(255,255,255,.14)}
.scene:has(.official-ui) .pathchip{background:rgba(255,255,255,.08);color:#e8edf5;
  border-color:rgba(255,255,255,.16);box-shadow:0 14px 30px rgba(0,0,0,.4)}
