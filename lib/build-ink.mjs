...
import { sceneArtPlan, sceneArt, stepStage, iconKeyFor, MOTIF_CSS } from "./scene-art.mjs";
...
export function buildInkHTML(pack) {
  ...
  const sceneStep = (i, tip, ink) => `
      <div class="stepwrap">
        ...
        ${stepStage(PAIR, { tip, art: ART_PLAN[i], iconSVG: `<span class="illo-wrap ink" style="color:${ink}">${illo(iconKeyFor(ART_PLAN[i]), i)}</span>`, markSVG: platformMark(pack.platform, ink, 40) })}
      </div>
      <div class="band" style="background:${ink}"><span>${tip.head}</span></div>`;
