...
export function buildInkHTML(pack) {
  ...
  const PRESENTER_IMAGE = `data:image/png;base64,${b64("public/presenter-real-v3.png")}`;
  // The presenter is a real photographed avatar, while the iPhone/app screen
  // remains the main graphic for the actual step.
  const presenter = (i, ink) => pack.platform === "news" ? "" : `
    <div class="guide guide-${i % 4}" aria-hidden="true" style="--guide:${ink}">
      <img class="guide-photo" src="${PRESENTER_IMAGE}" alt=""/>
...
.guide{position:absolute;z-index:4;left:34px;bottom:150px;width:210px;text-align:center;pointer-events:none;filter:drop-shadow(0 14px 14px rgba(20,25,35,.18));transform-origin:50% 90%}
.guide-photo{display:block;width:210px;height:330px;object-fit:cover;object-position:24% 85%;border-radius:104px 104px 42px 42px;border:5px solid rgba(255,255,255,.92);box-shadow:0 18px 35px rgba(20,25,35,.2)}.guide span{display:inline-block;margin-top:-20px;padding:6px 14px;border-radius:999px;background:#fff;color:#16233a;font-size:20px;font-weight:800;box-shadow:0 7px 16px rgba(20,25,35,.13)}
...
.compact .guide{width:172px;bottom:118px}.compact .guide-photo{width:172px;height:270px}.compact .guide span{font-size:17px}
/* --- step scene --- */
...
TIPS.forEach(function(t){
  ...
  if(document.querySelector(id+" .guide")){
    tl.fromTo(id+" .guide-photo",{y:38,scale:.78,opacity:0},{y:0,scale:1,opacity:1,duration:.42,ease:"back.out(1.7)"},at+.18);
    tl.to(id+" .guide-photo",{y:-8,rotation:.8,duration:.72,yoyo:true,repeat:Math.max(1,Math.floor(t.dur/.72)-1),ease:"sine.inOut"},at+.7);
  }
