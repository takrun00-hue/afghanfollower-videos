...
export function buildInkHTML(pack) {
  ...
  const DESIGN = pack.kind === "german-lesson"
    ? "german-flashcard"
  ...
  const shapeOf = (photo) => {
    ...
    if (ar > 0.85) return { cls: "panel", ar: 0.75 };
    return { cls: "panel", ar };
  };
  ...
  const frameShape = (photo, forcedAr) => (forcedAr ? { cls: "panel", ar: forcedAr } : shapeOf(photo));
...
<div id="root" class="${COMPACT ? 'compact' : ''} ${pack.platform === "news" ? "newsvid" : ""} design-${DESIGN}" data-composition-id="main" data-start="0" data-width="1080" data-height="1920" data-duration="${TOTAL}">
