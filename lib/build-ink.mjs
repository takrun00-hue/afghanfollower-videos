...
export function buildInkHTML(pack) {
  ...
  const sceneStep = (i, tip, ink) => `
      <div class="stepwrap">
        ...
        ${tip.video ? null : tip.photo
          ? `<div class="official-ui ${shapeOf(tip.photo).cls} focus-${tip.photoFocus || "default"}" style="--ar:${shapeOf(tip.photo).ar.toFixed(3)}"><span class="shot"><img src="${photoSrc(tip.photo)}" alt="${tip.photoAlt || "Official app interface"}"/></span><span class="focusframe"></span><span class="tapdot"></span></div>`
          : pack.photos?.[i]
  ...
  const scenes = pack.tips
    ...
    <div class="world" id="w${i + 2}">
      ${COMPACT || tip.video || pack.platform === "news" ? "" : `<div class="kick">${pack.kicker || "قابلیت"}</div>`}
${inner}
      ${COMPACT
  ...
  const tipsData = pack.tips.map((tip, i) => ({
    id: "s" + (i + 2), at: +tipStart(i).toFixed(3), dur: +tipLen(i).toFixed(3), i, k: kindOf(tip),
  }));
...
.hookscene .paper{background:
  radial-gradient(120% 80% at 78% 8%, ${PAIR[1]}30, transparent 60%),
  radial-gradient(110% 70% at 8% 96%, ${PAIR[0]}22, transparent 62%),
  linear-gradient(168deg, #fff 0%, ${PAIR[1]}12 58%, ${PAIR[0]}1a 100%)}
...
/* The ground a device stands on.

   Every reference video builds depth behind its content — paper grain, a
   grid, a dark studio — and none of them leaves the page white. A slide
   carrying a real interface gets the same: a deeper wash toward the corners,
   a faint grid that gives the eye a scale reference, and a broad glow behind
   the device so it reads as lit rather than pasted. */
.scene:has(.official-ui) .paper{
  background:
    radial-gradient(circle at 50% 34%, color-mix(in srgb,var(--ink) 30%,transparent), transparent 58%),
    radial-gradient(circle at 84% 8%, rgba(255,255,255,.55), transparent 46%),
    linear-gradient(178deg,#f7f8fb 0%,#eceff6 42%,#dfe4ef 100%)}
.scene:has(.official-ui) .paper::before{content:"";position:absolute;inset:0;opacity:.5;
  background-image:linear-gradient(rgba(60,74,102,.09) 1px,transparent 1px),
    linear-gradient(90deg,rgba(60,74,102,.09) 1px,transparent 1px);
  background-size:64px 64px;
  mask-image:radial-gradient(circle at 50% 42%, #000 12%, transparent 72%)}

/* A real interface, presented as a device rather than as a picture.

   The old treatment put a screenshot on a pale card in the middle of a pale
   page: the image floated, the page read as unfinished, and nothing told the
   viewer they were looking at a phone. The reference channels never do this —
   their content always sits in a hard-edged panel with a real shadow, over a
   background that has been designed rather than left blank.

   So: a dark bezel with a speaker slot, the screenshot filling the display,
   a soft riser panel behind it for depth, and a shadow that puts the whole
   thing above the page instead of on it. */
/* A slide carrying a device does not need 470px of reserved floor. That
   padding leaves the bottom third empty — the dead space that reads as
   unfinished — so a media slide rebalances it and spends the room on the
   device instead, which is the thing the viewer came to look at. */
...
.official-ui.phone{transform-origin:50% 50%}
...
.official-ui.phone{width:452px;height:816px}
...
/* --- outro --- */
/* The news footnote: legible if looked for, never loud enough to read aloud. */
.newsfoot{margin-top:26px;font-size:22px;font-weight:600;letter-spacing:.02em;
  color:rgba(70,78,92,.62);direction:rtl}
  ...
  <section id="s1" class="clip hookscene" data-start="0" data-duration="${HOOK}" data-track-index="1">
    ...
    <div class="world hookworld" id="w1">
      ...
      <div class="hookpreview mystery">
        ...
        <span class="spot" style="--ring:${PAIR[1]}"></span>
        <span class="qmark" style="background:${PAIR[1]}">؟</span>
      </div>
...
${brollClips}
  ...
  <section id="sOut" class="clip outroscene" data-start="${(HOOK + DURS.reduce((a, d) => a + d, 0)).toFixed(3)}" data-duration="${OUTRO}" data-track-index="1">
    ...
    <div class="world" id="wOut">
      ...
      ${pack.platform === "news" && pack.source && pack.source !== "—"
        ? `<div class="newsfoot">${plain(String(pack.source).split("·")[0].trim())}</div>` : ""}
...
tl.fromTo("#s1 .veil",{opacity:0},{opacity:1,duration:.5,ease:"power2.out"},.30);
tl.fromTo("#s1 .spot",{scale:.35,opacity:0},{scale:1,opacity:.92,duration:.5,ease:"back.out(2)"},.42);
tl.to("#s1 .spot",{scale:1.12,duration:.6,yoyo:true,repeat:3,ease:"sine.inOut"},.95);
tl.fromTo("#s1 .qmark",{scale:0,rotation:-24},{scale:1,rotation:0,duration:.5,ease:"back.out(2.4)"},.72);
tl.to("#s1 .qmark",{y:-12,duration:.9,yoyo:true,repeat:2,ease:"sine.inOut"},1.30);
...
TIPS.forEach(function(t){
  ...
  tl.fromTo(id+" .official-ui",{scale:1},{scale:1.055,duration:Math.max(1.2,t.dur-.2),ease:"none"},at+.55);
