...
export function buildInkHTML(pack) {
  ...
  const sceneStep = (i, tip, ink) => `
      <div class="stepwrap">
        ...
        ${pack.photos?.[i]
          ...
          : (tip.screen || tip.path || tip.ui) && ["tiktok", "instagram"].includes(pack.platform)
            ? iphoneFrame(stepStage(PAIR, { tip, art: ART_PLAN[i], platform: pack.platform, iconSVG: `<span class="illo-wrap ink" style="color:${ink}">${illo(iconKeyFor(ART_PLAN[i]), i)}</span>`, markSVG: platformMark(pack.platform, ink, 40) }))
  ...
  // A small animated presenter supports the spoken instruction without
  // replacing the real iPhone/app screen. Different poses avoid a cloned
  // sticker feeling while the content-specific screen remains the hero.
  const presenter = (i, ink) => pack.platform === "news" ? "" : `
    <div class="guide guide-${i % 4}" aria-hidden="true" style="--guide:${ink}">
      <svg viewBox="0 0 190 290" fill="none" xmlns="http://www.w3.org/2000/svg">
        <ellipse class="guide-shadow" cx="98" cy="270" rx="64" ry="12" fill="rgba(20,25,35,.18)"/>
        <path class="guide-body" d="M52 250c6-56 27-82 47-82s42 26 47 82" fill="var(--guide)"/>
        <path class="guide-shirt" d="M73 179c8 10 36 10 52 0l10 70H63l10-70Z" fill="#fff"/>
        <path class="guide-arm arm-a" d="M67 193c-22 12-31 36-23 48 7 10 19 4 27-14" stroke="var(--guide)" stroke-width="16" stroke-linecap="round"/>
        <path class="guide-arm arm-b" d="M129 192c24 6 36 24 31 40-4 12-18 10-26-6" stroke="var(--guide)" stroke-width="16" stroke-linecap="round"/>
        <rect class="guide-phone" x="142" y="203" width="29" height="48" rx="6" fill="#16233a" transform="rotate(18 142 203)"/>
        <ellipse class="guide-face" cx="99" cy="127" rx="39" ry="47" fill="#D79B77"/>
        <path class="guide-hair" d="M59 129c-3-49 23-75 49-75 33 0 51 29 43 79-8-21-20-34-42-35-17 0-32 11-50 31Z" fill="#202738"/>
        <path class="guide-hair" d="M60 117c-8 39-1 61 13 76l12-31-4-49Z" fill="#202738"/>
        <circle cx="85" cy="128" r="4" fill="#202738"/><circle cx="114" cy="128" r="4" fill="#202738"/>
        <path class="guide-mouth" d="M91 151q8 7 17 0" stroke="#8D3F46" stroke-width="4" stroke-linecap="round"/>
        <circle cx="72" cy="145" r="6" fill="#E98792" opacity=".45"/><circle cx="126" cy="145" r="6" fill="#E98792" opacity=".45"/>
      </svg>
  ...
  const scenes = pack.tips
    .map((tip, i) => {
      ...
      const usesPhoneGuide = (tip.screen || tip.path || tip.ui) && ["tiktok", "instagram"].includes(pack.platform);
      const inner =
...
/* presenter stays secondary to the real app screen and varies her pose per slide */
.guide{position:absolute;z-index:4;left:34px;bottom:150px;width:205px;text-align:center;pointer-events:none;filter:drop-shadow(0 14px 14px rgba(20,25,35,.18));transform-origin:50% 90%}
.guide svg{display:block;width:205px;height:312px}.guide span{display:inline-block;margin-top:-20px;padding:6px 14px;border-radius:999px;background:#fff;color:#16233a;font-size:20px;font-weight:800;box-shadow:0 7px 16px rgba(20,25,35,.13)}
.guide-1{left:auto;right:34px;transform:scaleX(-1)}.guide-1 span{transform:scaleX(-1)}
.guide-2{left:52px;bottom:134px;transform:scale(.92)}.guide-3{left:auto;right:48px;bottom:136px;transform:scale(-.92,.92)}.guide-3 span{transform:scaleX(-1)}
.compact .guide{width:172px;bottom:118px}.compact .guide svg{width:172px;height:262px}.compact .guide span{font-size:17px}
/* --- step scene --- */
...
.hookphoto{width:500px;height:440px;transform:rotate(-3deg)}
...
<div id="root" class="${COMPACT ? 'compact' : ''} ${pack.platform === "news" ? "newsvid" : ""}" data-composition-id="main" data-start="0" data-width="1080" data-height="1920" data-duration="${TOTAL}">
  ...
  <section id="s1" class="clip hookscene" data-start="0" data-duration="${HOOK}" data-track-index="1">
    ...
    <div class="world hookworld" id="w1">
      ...
      ${pack.platform === "news" ? "" : `<div class="hookblock" style="background:${PAIR[1]}">
      ...
      <div class="hookpreview">
        ${pack.platform === "news" && (pack.hookPhoto || pack.photos?.[0])
          ...
          : pack.platform !== "news" && pack.hookPhoto
            ? `<div class="newsphoto tutorialphoto hookphoto"><img src="${pack.hookPhoto.dataURI || pack.hookPhoto.src || pack.hookPhoto}" alt="راهنمای Video Assistant"/></div>`
          : HOOK_BRAND
            ? toolCard(HOOK_BRAND)
            : HOOK_SCREEN
...
TIPS.forEach(function(t){
  ...
  // The guide appears once the slide is established, then speaks with subtle
  // mouth pulses while the measured narration plays. Her SVG is animated, not
  // the positioned parent, so the four authored poses remain intact.
  if(document.querySelector(id+" .guide")){
    tl.fromTo(id+" .guide svg",{y:38,scale:.78,opacity:0},{y:0,scale:1,opacity:1,duration:.42,ease:"back.out(1.7)"},at+.18);
    tl.to(id+" .guide svg",{y:-8,duration:.72,yoyo:true,repeat:Math.max(1,Math.floor(t.dur/.72)-1),ease:"sine.inOut"},at+.7);
    tl.fromTo(id+" .guide-mouth",{scaleY:.22,transformOrigin:"50% 50%"},{scaleY:1,duration:.14,yoyo:true,repeat:Math.max(3,Math.floor(t.dur/.34)),ease:"steps(1)"},at+.46);
    tl.fromTo(id+" .guide-phone",{rotation:-8,transformOrigin:"50% 50%"},{rotation:8,duration:.48,yoyo:true,repeat:Math.max(2,Math.floor(t.dur/.96)),ease:"sine.inOut"},at+.56);
  }
