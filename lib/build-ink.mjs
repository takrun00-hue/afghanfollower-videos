...
import { motifFor, heroGraphic, backdropIconFor, MOTIF_CSS } from "./motifs.mjs";
...
export function buildInkHTML(pack) {
  ...
  const MOTIF = motifFor(pack);
  const HERO = MOTIF !== "generic";
  const BACKICON = backdropIconFor(MOTIF);
  ...
  const scenes = pack.tips
    ...
    <span class="backdrop" style="color:${ink}">${illo(HERO ? BACKICON : tip.icon, i)}</span>
...
function animateHero(R, at){
  ...
  var dr=gsap.utils.toArray(R+" .mdraw > *");
  if(dr.length) tl.fromTo(dr,{strokeDashoffset:1},{strokeDashoffset:0,duration:.8,stagger:.035,ease:"power2.out"},at+.1);
  tl.fromTo(R+" .mknob",{x:0},{x:100,duration:.55,ease:"back.out(1.5)"},at+.6);
  tl.to(R+" .mpulse",{scale:1.14,duration:.5,yoyo:true,repeat:6,ease:"sine.inOut"},at+.5);
}
...
TIPS.forEach(function(t){
  ...
  if(t.k==="step"){
    // 1. the whole stage lands with a spring — depth, not a flat cut
    tl.fromTo(id+" .stepwrap",{y:64,scale:.84,opacity:0},
              {y:0,scale:1,opacity:1,duration:.58,ease:"back.out(1.5)"},at+.08);
    // 2. CONTAINER MORPH — the card starts compact and grows to full height,
    //    revealing its own contents as it opens. This is the single move that
    //    gives the reference its product-demo feel.
    tl.fromTo(id+" .pmock:not(.win)",{height:250},{height:660,duration:.74,ease:"expo.out"},at+.16);
    tl.fromTo(id+" .pmock.win",{height:210},{height:520,duration:.74,ease:"expo.out"},at+.16);
    // 3. staggered reveal — every row rises into place behind the growing edge
    tl.fromTo(id+" .ptop",{opacity:0,y:-16},{opacity:1,y:0,duration:.3,ease:"power2.out"},at+.3);
    tl.fromTo(id+" .prow, "+id+" .pcmt",{opacity:0,y:46,scale:.96},
              {opacity:1,y:0,scale:1,duration:.4,stagger:.085,ease:"back.out(1.5)"},at+.38);
    tl.fromTo(id+" .pmedia, "+id+" .pcompare, "+id+" .pduo",{opacity:0,y:34,scale:.93},
              {opacity:1,y:0,scale:1,duration:.44,ease:"back.out(1.4)"},at+.4);
    tl.fromTo(id+" .pcta",{opacity:0,y:36},{opacity:1,y:0,duration:.42,ease:"back.out(1.7)"},at+.6);
    ...
    // the glossy sphere drops in and then keeps its own slow float, offset from
    // the card's — two objects moving at different rates is what reads as depth
