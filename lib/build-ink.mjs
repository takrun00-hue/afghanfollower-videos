...
<div id="root" class="${COMPACT ? 'compact' : ''} ${pack.platform === "news" ? "newsvid" : ""}" data-composition-id="main" data-start="0" data-width="1080" data-height="1920" data-duration="${TOTAL}">
  ...
  <section id="s1" class="clip scene k-step hookscene" data-start="0" data-duration="${HOOK}" data-track-index="1" style="--ink:${PAIR[1]}">
    ...
    <span class="charwrap">${characterSVG({ pose: poseFor(pack.hook.ask || ""), ink: PAIR[1] })}</span>
    <div class="world" id="w1">
      <div class="kick">${pack.kicker || "قابلیت"}</div>
      <div class="stepwrap">
        ${HOOK_MEDIA
          ? `<div class="official-ui ${shapeOf(HOOK_MEDIA).cls} focus-default" style="--ar:${shapeOf(HOOK_MEDIA).ar.toFixed(3)}">${shapeOf(HOOK_MEDIA).cls === "phone" ? '<span class="island"></span><span class="btn-l"></span><span class="btn-r"></span>' : ""}<span class="shot"><img src="${photoSrc(HOOK_MEDIA)}" alt="${HOOK_MEDIA_ALT}"/></span></div>`
