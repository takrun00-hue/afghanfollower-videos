...
import { motifFor, conceptGraphic, stepStage, backdropIconFor, MOTIF_CSS } from "./motifs.mjs";
...
export function buildInkHTML(pack) {
  ...
  const sceneStep = (i, tip, ink) => `
      <div class="stepwrap">
        ...
        ${stepStage(MOTIF, PAIR, { step: i, total: n, iconSVG: `<span class="illo-wrap ink" style="color:${ink}">${illo(tip.icon, i)}</span>`, markSVG: platformMark(pack.platform, ink, 40) })}
      </div>
      <div class="band" style="background:${ink}"><span>${tip.head}</span></div>`;
