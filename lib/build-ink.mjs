...
export function buildInkHTML(pack) {
  ...
  // Keep the visual guide in this active builder. A feature may provide a
  // path or lightweight UI data instead of a complete screen object; convert
  // it here so the rendered cloud video still shows one clear iPhone page.
  const screenTip = (tip) => {
    ...
    const path = String(tip.path || "").split(">").map((x) => x.trim()).filter(Boolean);
    return path.length
      ? { ...tip, screen: { title: path[0], rows: path.slice(1).concat(["Settings", "Help"]).slice(0, 3), hit: Math.max(0, Math.min(path.length - 2, 2)) } }
      : tip;
  };
