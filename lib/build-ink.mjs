...
import { motifFor, conceptGraphic, backdropIconFor, MOTIF_CSS } from "./motifs.mjs";
...
export function buildInkHTML(pack) {
  ...
  const sceneStep = (i, tip, ink) => `
      <div class="stepwrap">
        ...
        <div class="stage" style="--ink:${ink}">
          <span class="stage-badge">${platformMark(pack.platform, ink, 40)}</span>
          ${conceptGraphic(MOTIF, PAIR, i, n)}
          <span class="stage-ico" style="color:${ink}">${illo(tip.icon, i)}</span>
        </div>
      </div>
      <div class="band" style="background:${ink}"><span>${tip.head}</span></div>`;
