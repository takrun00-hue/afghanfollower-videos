...
const SVG_HEAD = /^\s*<\?xml[^>]*>\s*<svg\b|^\s*<svg\b/i;
...
export function imageType(path) {
  ...
  // Generated fallback art is stored as SVG so it remains sharp in a vertical
  // video. It is accepted only when it is explicitly tagged as generated in
  // visualEvidence; this check merely verifies that it really is image data.
  try {
    const text = readFileSync(path, "utf8").slice(0, 512);
    if (SVG_HEAD.test(text)) return "svg";
  } catch {}
  return "";
}
...
export function imageSize(path) {
  ...
  // SVG is a first-class image format for the local generated fallback. The
  // document must declare dimensions; no guessed default size is accepted.
  try {
    const head = buf.toString("utf8", 0, Math.min(buf.length, 2048));
    if (SVG_HEAD.test(head)) {
      const width = Number(head.match(/\bwidth=["'](\d+(?:\.\d+)?)/i)?.[1]);
      const height = Number(head.match(/\bheight=["'](\d+(?:\.\d+)?)/i)?.[1]);
