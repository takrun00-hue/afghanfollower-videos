...
// hard platform constraint, not a design choice — so the only way for a
// Recovery Loop to keep trying past the budget of a SINGLE job run is for
// that job, before it exits, to trigger its own successor and hand it the
// failure evidence. This script is that hand-off: it runs as a `failure()`
// step in .github/workflows/news-scan.yml, reads the marker
// german-lesson-build.mjs wrote when lib/recovery-engine.mjs's
// RecoveryExhausted fired, and — bounded, never infinite — pushes another
// attempt via the same push-trigger file daily.yml/news-scan.yml already
// use for their hourly reliability pings, with no human or agent needing
// to notice the failure first.
//
...
if (isMain) {
  ...
  console.log(`Chained an automatic Recovery Loop retry for "${marker.unit}" (attempt ${attempts}/${MAX_CHAIN_RETRIES}) — no human or agent action needed; news-scan.yml's push trigger will start the next attempt.`);
}
