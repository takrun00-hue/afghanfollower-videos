...
export const SOURCES = {
  ...
  amalFra:  { name: "امل فرانکفورت",        domain: "amalfrankfurt.de",  fa: true, germany: true },
  bbcDari:  { name: "BBC دری",              domain: "bbc.com",           fa: true },
  dwFa:     { name: "DW فارسی/دری",         domain: "dw.com",            fa: true, germany: true },
  // Public broadcaster's Persian service. It is useful alongside Amal for
  // practical Germany coverage, while still being filtered to migrant impact.
  wdrFa:    { name: "WDR for you فارسی",    domain: "wdr.de",           fa: true, germany: true },
...
// Same-story detection, shared by anything that scans for news: a URL match
// only catches the same page twice — the same deportation flight written up
// by Amal, DW and BBC is three different URLs and would otherwise arrive as
// three "new" stories.
const STOP = new Set(["از", "به", "در", "را", "که", "با", "این", "برای", "است", "شد",
  "شده", "های", "یک", "بر", "تا", "هم", "و", "بود", "کرد", "می", "خود", "بین", "دو"]);

// "گروه دیگری" and "گروهی دیگر" are the same story told twice, so the plural
// and possessive endings come off before comparing.
const stem = (w) => w.replace(/(های|ها|ان|ی)$/, "") || w;

// Reduces a headline to its sorted, stemmed content words for comparison.
export const keyOf = (title) =>
  // "…اخراج شدند - اطلاعات روز" — the outlet's own name at the end is noise
  // that pushes two reports of one event below the similarity threshold.
...
// "افغان" and "افغانستان" are the same subject, and one stem rule cannot
// flatten both, so words also count as shared when one is a prefix of the other.
const akin = (x, y) =>
  x === y || (x.length >= 3 && y.startsWith(x)) || (y.length >= 3 && x.startsWith(y));

// a and b are already keyOf()'d word lists.
export const sameStory = (a, b) => {
