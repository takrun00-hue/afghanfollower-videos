...
${inner}
  ...
  const scenes = pack.tips
    .map((tip, i) => pack.tierList ? tierScene(i, tip) : isDialogue ? dialogueScene(i, tip) : tipScene(i, tip))
    .join("\n");
...
/* ---------- faceless tier-list ---------- */
${BRAND_LOGO_CSS}
.tier-hook .hookwrap{gap:56px}
.tier-hook .bg{background:radial-gradient(600px at 50% 0%,#7c3aed 0%,#000 60%)}
.tier-badge-pill{font-size:32px;padding:12px 28px;margin-bottom:40px}
.tier-hook-h1{text-align:center;line-height:1.1;direction:rtl}
.tier-stat{color:#ff3b30;font:900 110px/1.1 "Baloo","Vazirmatn",sans-serif;display:block;
  transform:scale(1.3);text-shadow:0 10px 40px rgba(255,59,48,.4)}
.tier-h1-l1{color:#fff;font:700 72px/1.2 "Vazirmatn",sans-serif;display:block;margin-top:18px}
.tier-h1-l2{color:#fff;font:900 82px/1.2 "Baloo","Vazirmatn",sans-serif;display:block}
.tier-sub{color:#aaa;font:700 42px/1.3 "Vazirmatn",sans-serif;margin-top:30px;direction:rtl}
.tier-columns{display:flex;gap:22px;margin-top:48px}
.tier-col{flex:1;height:220px;border-radius:24px;display:flex;align-items:flex-end;justify-content:center;
  padding-bottom:22px;border:3px solid rgba(255,255,255,.14)}
.tier-col.bad{background:linear-gradient(180deg,rgba(255,59,59,.05),rgba(255,59,59,.28))}
.tier-col.good{background:linear-gradient(180deg,rgba(255,217,61,.05),rgba(255,217,61,.28))}
.tier-col.great{background:linear-gradient(180deg,rgba(48,209,88,.05),rgba(48,209,88,.28))}
.tier-col-label{font:900 34px/1 "Baloo","Vazirmatn",sans-serif;letter-spacing:.06em}
.tier-col.bad .tier-col-label{color:#FF3B3B}
.tier-col.good .tier-col-label{color:#FFD93D}
.tier-col.great .tier-col-label{color:#30D158}
...
<div id="root" data-composition-id="main" data-start="0" data-width="1080" data-height="1920" data-duration="${TOTAL}">
  ...
  <section id="s1" class="clip${pack.tierList ? " tier-hook" : ""}" data-start="0" data-duration="${HOOK}" data-track-index="1">
    ...
    <div class="wrap hookwrap" id="s1c">
      ...
      <h1 class="tier-hook-h1">
        <span class="tier-stat">${pack.hook.stat}</span>
        <span class="tier-h1-l1">${pack.hook.l1}</span>
        <span class="tier-h1-l2">${pack.hook.l2}</span>
      </h1>
      <p class="tier-sub">${pack.hook.sub}</p>
...
// tier-list hook: badge, then the stat line, then the two payoff lines, then
// the subtext, then the BAD/GOOD/GREAT columns rise in from below.
tl.fromTo("#s1 .tier-badge-pill",{scale:0,rotate:-14},{scale:1,rotate:0,duration:.42,ease:"back.out(2.6)"},.10);
tl.fromTo("#s1 .tier-stat",{scale:.7,opacity:0},{scale:1.3,opacity:1,duration:.5,ease:"back.out(2.2)"},.22);
tl.fromTo("#s1 .tier-h1-l1,#s1 .tier-h1-l2",{y:40,opacity:0},{y:0,opacity:1,duration:.4,stagger:.08,ease:"power3.out"},.5);
tl.fromTo("#s1 .tier-sub",{opacity:0},{opacity:1,duration:.4,ease:"power2.out"},.85);
tl.from("#s1 .tier-col",{y:100,opacity:0,duration:.6,stagger:.1,ease:"back.out(1.7)"},1.05);
ken("#s1c",0,${HOOK});
