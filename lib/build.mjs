...
const ICONS = {
  ...
  // Friendly robot head — the tier-list hook's mascot mark. Eyes and antenna
  // are separate elements (not baked into one path) specifically so they can
  // be targeted and animated independently (blink, glow) in the timeline.
  robot: (c) => S(`<rect x="6" y="38" width="12" height="20" rx="6" fill="${c}"/><rect x="70" y="38" width="12" height="20" rx="6" fill="${c}"/>
    <circle cx="44" cy="10" r="6" fill="${c}"/><rect x="41" y="14" width="6" height="12" fill="${c}"/>
    <rect x="14" y="24" width="60" height="50" rx="18" fill="${c}"/>
    <circle class="robot-eye" cx="31" cy="48" r="8" fill="#05081f"/><circle class="robot-eye" cx="57" cy="48" r="8" fill="#05081f"/>
    <rect x="27" y="62" width="34" height="6" rx="3" fill="#05081f"/>`),
};
...
export function buildHTML(pack) {
  ...
  // Faceless tier-list slide: a category header, then each real app's name
  // and mark popping into a pill underneath it, then the capability chips
  // the narration is also saying — "معرفی فقط اسم را می‌گیرد، از کارکردها
  // نمی‌گوید": the pills named the apps but never said what they do, so a
  // viewer had no reason to remember one. tip.capabilities is the same
  // array content.mjs hands to MiniMax, not separate on-screen copy — the
  // on-screen and spoken text can't drift apart because there is only one
  // source array.
  const tierScene = (i, tip) => {
    ...
    const pills = tip.tools
      ...
      .join("\n");
    const caps = Array.isArray(tip.capabilities) ? tip.capabilities : [];
    const capChips = caps
      .map((c) => `<div class="tier-cap" style="--acc:${a}"><span class="tier-cap-mark">${(ICONS.sparkle)(a)}</span><span class="tier-cap-text">${c}</span></div>`)
      .join("\n");
    ...
    <div class="tier-stage" id="s${i + 2}c">
      ...
      ${caps.length ? `<div class="tier-caps">${capChips}</div>` : ""}
    </div>
  </section>`;
  };
...
${inner}
  ...
  const tipsData = pack.tips.map((tip, i) => ({
    ...
    pillCount: Array.isArray(tip.tools) ? tip.tools.length : 0,
    i,
  }));
...
/* A soft shadow alone read fine on a dark scene, then a light canvas (added
   for the tier-list hook) made it unreadable — this overlay spans every
   scene the video has, dark or light, so the wordmark needs its own solid
   backdrop rather than depending on what happens to sit behind it. */
.brandchip .bt{font-weight:900;font-size:34px;color:#fff;background:rgba(5,8,25,.55);
  backdrop-filter:blur(4px);padding:8px 18px;border-radius:999px}
...
/* The warm reference was the canvas around the phone, not the phone's own
   screen — the phone screen stays the dark purple-to-black it was. */
/* Same "blue-leaning white, not full blue, not full white" spec given for
   the phone screen — this time for the canvas, which is what was asked. */
.tier-hook .bg{background:radial-gradient(900px 700px at 50% 30%,#F4F9FF 0%,#DCEBFA 55%,#C7DEF2 100%)}
...
/* Reverted to its original state — the phone itself is not to be touched;
   only the canvas around it changes. */
.iphone15-screen{background:radial-gradient(600px at 50% 0%,#7c3aed 0%,#000 70%);
...
/* Sized for a 380px-wide phone screen, not the full 1080px frame — a flat
   ...
   "متن را مثل دیگر سلایدها بنویس" — the small teal chat-bubble box read as
   a UI notification, not a headline; every other slide's .headline sits
   directly on its scene background with a drop shadow for legibility, so
   this drops the box/background and matches that treatment (bigger,
   bolder, Baloo — the same display face .headline uses). */
.tier-typed{font-family:"Baloo","Vazirmatn",sans-serif;font-weight:800;line-height:1.35;
  text-align:center;direction:rtl;flex:none;text-shadow:0 6px 24px rgba(0,0,0,.5)}
.tier-part{color:#fff}
.tier-part-cursor{font-weight:400;margin-right:.05em;opacity:0}
.tier-part-plain{color:#fff;font-size:42px;font-weight:800;display:inline-block}
.tier-part-purple{color:#c4b5fd;font-size:54px;font-weight:900;display:inline-block;
  text-shadow:0 0 26px #a78bfa88}
.tier-part-green{color:#34d399;font-size:44px;font-weight:800;display:inline-block;
  text-shadow:0 0 24px #34d399aa}
...
.tier-caps{display:flex;flex-wrap:wrap;gap:16px;justify-content:center;max-width:700px}
.tier-cap{display:flex;align-items:center;gap:12px;direction:rtl;
  background:color-mix(in srgb,var(--acc) 16%,transparent);border:1.5px solid var(--acc);
  border-radius:20px;padding:14px 24px}
.tier-cap-mark svg{width:34px;height:34px;flex:none}
.tier-cap-text{font:700 30px/1.4 "Vazirmatn",sans-serif;color:#fff}
/* real tool marks floating around the hook's phone — drawn from this
   pack's own selected categories, not decoration with no connection to
   what the video is actually about */
.hook-orbit{position:absolute;inset:0;z-index:1;pointer-events:none}
.hook-orbit-icon{position:absolute;opacity:0}
.hook-orbit-icon .blogo{width:108px;height:108px;border-radius:28px;
  box-shadow:0 16px 34px rgba(0,0,0,.4);border:3px solid rgba(255,255,255,.5)}
.hoi-0{left:7%;top:22%}
.hoi-1{right:7%;top:17%}
.hoi-2{left:9%;top:64%}
.hoi-3{right:9%;top:68%}
/* "یک عکسی رباتیک با انیمیشن اضافه کن" — a mascot, not one more static
   label. Taken out of the flex flow (absolute) so it floats above the
   phone instead of pushing the phone's own height into the hook's fixed
   padding box — see the tier-typed comment above for why that box is
   tight. Eye-blink and idle-float are driven from the timeline, not CSS
   @keyframes, so they stay seek-safe like everything else in this file. */
.hook-robot{position:absolute;left:50%;top:120px;
  z-index:5;opacity:0;filter:drop-shadow(0 16px 34px rgba(0,0,0,.45))}
.hook-robot svg{width:168px;height:168px}
</style>
</head>
<body>
<div id="root" data-composition-id="main" data-start="0" data-width="1080" data-height="1920" data-duration="${TOTAL}">
  ...
  <section id="s1" class="clip${pack.tierList ? " tier-hook" : ""}" data-start="0" data-duration="${HOOK}" data-track-index="1">
    ...
    <div class="wrap hookwrap" id="s1c">
      ${pack.tierList ? `
      <div class="hook-robot" aria-hidden="true">${(ICONS.robot)("#22d3ee")}</div>
      <div class="iphone15">
      ...
      </div>${(pack.hook.previewTools && pack.hook.previewTools.length) ? `
      <div class="hook-orbit">${pack.hook.previewTools.map((name, i) => `<span class="hook-orbit-icon hoi-${i}">${brandLogo(name)}</span>`).join("")}</div>` : ""}` : `
...
// robot mascot: drops in with a bounce right after the phone, then idles
// (gentle bob + rotation) and blinks on a finite, deterministic schedule —
// no CSS @keyframes / repeat:-1 anywhere here, matching the rest of the file.
tl.fromTo("#s1 .hook-robot",{xPercent:-50,opacity:0,y:-60,scale:.5,rotate:-10},{xPercent:-50,opacity:1,y:0,scale:1,rotate:0,duration:.5,ease:"back.out(2.2)"},.22);
tl.to("#s1 .hook-robot",{y:-14,rotate:3,duration:1.3,yoyo:true,repeat:Math.max(1,Math.ceil((${HOOK}-.8)/1.3)),ease:"sine.inOut"},.75);
(function(){
  var blinkAt = .9, endAt = ${HOOK} - .3;
  while (blinkAt < endAt) {
    tl.to("#s1 .robot-eye",{scaleY:.15,duration:.06,ease:"power1.in"},blinkAt)
      .to("#s1 .robot-eye",{scaleY:1,duration:.09,ease:"power1.out"},blinkAt+.06);
    blinkAt += 1.7;
  }
})();
...
// real tool marks pop in around the phone, then drift gently for the rest
// of the hook — finite repeat count derived from HOOK, never -1.
tl.fromTo("#s1 .hook-orbit-icon",{scale:.3,opacity:0,rotate:-16},{scale:1,opacity:1,rotate:0,duration:.45,stagger:.12,ease:"back.out(2.2)"},.55);
tl.to("#s1 .hoi-0",{y:"-=20",duration:1.6,yoyo:true,repeat:Math.max(1,Math.ceil((${HOOK}-1)/1.6)),ease:"sine.inOut"},1.1);
tl.to("#s1 .hoi-1",{y:"+=18",duration:1.4,yoyo:true,repeat:Math.max(1,Math.ceil((${HOOK}-1)/1.4)),ease:"sine.inOut"},1.15);
tl.to("#s1 .hoi-2",{y:"-=16",duration:1.8,yoyo:true,repeat:Math.max(1,Math.ceil((${HOOK}-1)/1.8)),ease:"sine.inOut"},1.2);
tl.to("#s1 .hoi-3",{y:"+=22",duration:1.5,yoyo:true,repeat:Math.max(1,Math.ceil((${HOOK}-1)/1.5)),ease:"sine.inOut"},1.25);
ken("#s1c",0,${HOOK});
...
TIPS.forEach(function(t){
  ...
  if (TIER) {
    ...
    // capability chips land once the pills are in — the narration is only
    // reaching this clause a beat after naming the apps, so the chips
    // shouldn't appear before them. Two chips now (was one), each with its
    // own small stagger so they don't pop in as a single flat block.
    var pillCount=(TIPS[t.i]&&TIPS[t.i].pillCount)||1;
    var capAt=at+.5+.42+.16*Math.max(0,pillCount-1)+.25;
    tl.fromTo(id+" .tier-cap",{y:36,opacity:0},{y:0,opacity:1,duration:.42,stagger:.14,ease:"power3.out"},capAt);
  } else if (t.lay === "stat") {
