...
/* Sized for a 380px-wide phone screen, not the full 1080px frame — the
   original 52px was copied from the flat full-width design and ran the
   longer combined sentence straight into the BAD/GOOD/GREAT chips. */
.tier-typed{color:#fff;font:800 30px/1.35 "Vazirmatn",sans-serif;text-align:center;
  direction:rtl;flex:none}
.tier-cursor{color:#fff;font-weight:400;margin-right:.08em}
    ...
    <div class="wrap hookwrap" id="s1c">
      ...
      <div class="iphone15">
        ...
        <div class="iphone15-screen">
          ...
          <h1 class="tier-typed" id="s1typed" data-text="${escAttr(`${pack.hook.stat} ${pack.hook.l1} ${pack.hook.l2} ${pack.hook.sub}`)}"><span class="tier-cursor">|</span></h1>
...
// single tween drives a 0..1 progress value; onUpdate slices the real string
// by that progress. GSAP re-evaluates onUpdate on every seek, including
// scrubbing backwards, so this is exactly as seek-safe as gsap.to({text:...})
// would have been.
(function(){
  var el = document.querySelector("#s1typed");
  if (!el) return;
  var full = el.getAttribute("data-text") || "";
  var chars = Array.from(full); // code-point aware: never splits an emoji
  var o = { n: 0 };
  tl.to(o, {
    n: chars.length, duration: 2, ease: "none",
    onUpdate: function(){
      var shown = chars.slice(0, Math.round(o.n)).join("");
      el.innerHTML = shown + '<span class="tier-cursor">|</span>';
    },
  }, .5);
})();
// the cursor blinks for the whole hook — a finite repeat count, never -1
tl.to("#s1 .tier-cursor",{opacity:0,duration:.5,yoyo:true,repeat:7,ease:"steps(1)"},.5);
tl.from("#s1 .tier-col",{y:50,opacity:0,duration:.5,stagger:.15,ease:"back.out(1.7)"},2.7);
ken("#s1c",0,${HOOK});
