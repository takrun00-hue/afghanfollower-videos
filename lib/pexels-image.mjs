 ...
 * @returns {Promise<Array<{image:string, title:string, url:string, photographer:string}>>}
...
export async function findPexelsImage(query, label) {
  ...
  for (const hit of hits) {
    ...
    const raw = `public/user-media/pexels-${id}.img`;
    ...
    // The same relevance bar every other source passes. A stock library
    // will happily return something loosely associated with the words;
    // "real photograph" is necessary, not sufficient.
    let relevant = false;
    try {
      relevant = await isRelevant(query, [label].filter(Boolean), { title: hit.title, url: hit.url, snippet: hit.title });
    } catch { relevant = false; }
    if (!relevant) continue;

    console.error(`   ℹ Pexels: «${query}» → ${named}${hit.photographer ? ` (photo: ${hit.photographer})` : ""}`);
