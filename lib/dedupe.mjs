...
// The registry is a compact semantic index. The delivery history is the
// durable editorial ledger and includes successful sends from older routes.
// A candidate is new only when it clears BOTH records.
const DELIVERY_HISTORY = ".content-history.json";
...
function deliveryEntries() {
  return readJSON(DELIVERY_HISTORY, []).map((entry) => ({
    ...entry,
    // Older records lack a full semantic fingerprint. Their stable id still
    // blocks exactly repeated content; the available topic/hook help catch
    // a renamed version of the same story as well.
    feature: String(entry.feature || entry.topic || entry.id || ""),
    topic: String(entry.topic || entry.feature || entry.id || ""),
    question: String(entry.hook || entry.question || ""),
    keyPoints: Array.isArray(entry.keyPoints) ? entry.keyPoints : [],
  }));
}

export function history(days = Infinity) {
  const cutoff = Number.isFinite(days) ? Date.now() - days * 86400000 : -Infinity;
  const all = [...readJSON(REGISTRY, []), ...deliveryEntries()]
    .filter((e) => (Date.parse(e.sentAt || "") || 0) > cutoff);
  // The same Telegram delivery lives in both files. Compare it once, but keep
  // genuine separate sends of the same subject: their matching id must block
  // any third attempt.
  const seen = new Set();
  return all.filter((e) => {
    const key = e.messageId ? `message:${e.messageId}` : `${e.id || ""}|${e.sentAt || ""}`;
...
export function check(candidate, { days = Infinity, alsoAgainst = [] } = {}) {
