...
import { realLogoData } from "./real-logo.mjs";
...
export function toolCard({ name, url, color = "#0e7490", tagline = "" }) {
  return `<div class="tcard" style="--brand:${color}">
    ...
    <div class="tbody">
      ${(() => {
        const real = realLogoData(name);
        return real
          ? `<span class="treal"><img src="${real}" alt="${esc(name)}"/></span>`
          : brandLogo(name, color);
      })()}
...
${BRAND_LOGO_CSS}
.treal{display:grid;place-items:center;width:132px;height:132px;border-radius:30px;
  background:#fff;box-shadow:0 10px 26px rgba(20,25,40,.16);overflow:hidden}
.treal img{max-width:96px;max-height:96px;object-fit:contain}
