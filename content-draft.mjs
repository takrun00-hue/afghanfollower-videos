...
async function announce(draft) {
  ...
  if (visualIssue) {
    ...
    extra.push(candidates
      ...
      : `<b>تصویر واقعی:</b> از منبع، تصویر قابل‌استفاده پیدا نشد. یک اسکرین‌شات واقعی از همان قابلیت را به‌صورت عکس با کپشن دقیق «${esc(draft.featureId)}» بفرستید، یا موضوع دیگری را انتخاب کنید؛ تا آن زمان ویدیویی ساخته نمی‌شود.`
    );
  } else {
